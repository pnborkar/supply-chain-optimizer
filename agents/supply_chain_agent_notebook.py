...
# MAGIC
# MAGIC Router → SQL Agent (gold Delta tables) or Graph Agent (Neo4j AuraDB)
# MAGIC
...
# MAGIC ```
# MAGIC Or set the widget values below to override.
...
class Neo4jConnector:
    ...
    # Maps each subgraph_type to a quick existence check query
    _EXISTENCE_CHECKS = {
    ...
    def project_subgraph(self, subgraph_type: str) -> int:
        # Check in-memory cache first (fast), then verify against Neo4j (reliable)
    ...
    def _project_supplier_risk(self) -> int:
        ...
        """)
        rows = run_sql(f"""
            SELECT supplier_id, part_id, COUNT(*) AS po_count, AVG(aging_days) AS avg_delay_days
...
# MAGIC
# MAGIC **Graph (relationships & impact)**
