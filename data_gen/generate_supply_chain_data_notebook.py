...
# MAGIC %md
# MAGIC # Supply Chain Synthetic Data Generator — Phase 1
# MAGIC
# MAGIC Generates realistic manufacturing supply chain data using **Spark + Faker + Pandas UDFs**.
# MAGIC
# MAGIC | Table | Rows | Description |
# MAGIC |-------|------|-------------|
# MAGIC | `suppliers` | 200 | Supplier companies |
# MAGIC | `parts` | 500 | Parts / components catalog |
# MAGIC | `facilities` | 50 | Manufacturing plants and warehouses |
# MAGIC | `bom` | ~2,000 | Bill of Materials (parent-child part relationships) |
# MAGIC | `purchase_orders` | 8,000 | POs from facilities to suppliers |
# MAGIC | `shipments` | ~12,000 | Physical shipments against POs |
# MAGIC
# MAGIC **Output:** `/Volumes/supplychain/supply_chain_raw/landing/<table>/`
...
# MAGIC %md ## Create Infrastructure
...
def gen_certifications(tiers: pd.Series) -> pd.Series:
    import random
    pool = {
        "Tier-1": ["ISO-9001, AS9100", "ISO-9001, IATF-16949, AS9100", "ISO-9001, ISO-14001, IATF-16949"],
...
def gen_carrier(ids: pd.Series) -> pd.Series:
    carriers = ["FastFreight LLC", "GlobalShip Express", "PacificCargo Co", "AtlasLogistics", "SwiftTransit"]
    probs    = [0.30, 0.30, 0.20, 0.12, 0.08]
    rng = np.random.default_rng(42)
...
tier_expr    = (F.when(F.col("_rt") < 0.15, "Tier-1").when(F.col("_rt") < 0.55, "Tier-2").otherwise("Tier-3"))
payment_expr = (F.when(F.col("_rp") < 0.10, 15).when(F.col("_rp") < 0.40, 30)
                 .when(F.col("_rp") < 0.70, 45).when(F.col("_rp") < 0.90, 60).otherwise(90))
...
print(f"suppliers: {spark.table(f'{TEMP_DB}.suppliers').count():,} rows → {VOLUME_PATH}/suppliers")

# COMMAND ----------

# MAGIC %md ## 2. Parts (500 rows)

# COMMAND ----------

cat_expr    = (F.when(F.col("_rc") < 0.30, "Raw Material").when(F.col("_rc") < 0.70, "Sub-Assembly").otherwise("Component"))
subcat_expr = (
    F.when(F.col("category") == "Raw Material",
           F.when(F.col("_rs") < 0.40, "Metal").when(F.col("_rs") < 0.70, "Polymer").otherwise("Composite"))
     .when(F.col("category") == "Sub-Assembly",
           F.when(F.col("_rs") < 0.50, "Powertrain").when(F.col("_rs") < 0.75, "Chassis").otherwise("Electronics"))
     .otherwise(F.when(F.col("_rs") < 0.40, "Mechanical").when(F.col("_rs") < 0.70, "Electrical").otherwise("Structural"))
)
uom_expr  = (F.when(F.col("category") == "Raw Material",
               F.when(F.col("_rum") < 0.40, "KG").when(F.col("_rum") < 0.70, "LB").otherwise("M")).otherwise("EA"))
lead_expr = (F.when(F.col("category") == "Raw Material", (F.rand(seed=14) * 14  + 1).cast("int"))
              .when(F.col("category") == "Sub-Assembly",  (F.rand(seed=15) * 60  + 14).cast("int"))
              .otherwise(                                  (F.rand(seed=16) * 30  + 3).cast("int")))
moq_expr  = (F.when(F.col("category") == "Raw Material", (F.rand(seed=17) * 900 + 100).cast("int"))
...
parts_df = (
    ...
    .withColumn("part_name",       gen_part_name(F.concat(F.col("category"), F.lit("|"), F.col("id").cast("string"))))
...
print(f"parts: {spark.table(f'{TEMP_DB}.parts').count():,} rows → {VOLUME_PATH}/parts")

# COMMAND ----------

# MAGIC %md ## 3. Facilities (50 rows)
...
ftype_expr    = (F.when(F.col("_rf") < 0.40, "Manufacturing").when(F.col("_rf") < 0.70, "Assembly").otherwise("Warehouse"))
capacity_expr = (
...
facilities_df = (
    ...
    .withColumn("facility_name",          F.concat(F.col("city"), F.lit(" "), F.col("facility_type"), F.lit(" Facility")))
...
print(f"facilities: {spark.table(f'{TEMP_DB}.facilities').count():,} rows → {VOLUME_PATH}/facilities")

# COMMAND ----------

# MAGIC %md ## 4. BOM (~2,000 rows)
...
print(f"Assembly-eligible parents: {n_asm}, total parts: {n_all}")
...
bom_df = (
    ...
    .withColumn("bom_id",
        F.concat(F.lit("BOM-"), F.lpad(
            F.row_number().over(Window.orderBy("parent_part_id", "child_part_id")).cast("string"), 5, "0")))
    .withColumn("quantity",        F.round(F.rand(seed=29) * 19 + 1, 0).cast("double"))
    .withColumn("unit_of_measure", F.lit("EA"))
    .withColumn("effective_date",  F.date_sub(F.current_date(), (F.rand(seed=30) * 730).cast("int")))
    .withColumn("is_active",       F.rand(seed=31) > 0.05)
    .select("bom_id", "parent_part_id", "child_part_id", "quantity", "unit_of_measure", "effective_date", "is_active")
)

bom_df.write.mode("overwrite").parquet(f"{VOLUME_PATH}/bom")
print(f"bom: written → {VOLUME_PATH}/bom")

# COMMAND ----------

# MAGIC %md ## 5. Purchase Orders (8,000 rows)

# COMMAND ----------

suppliers_idx  = (spark.table(f"{TEMP_DB}.suppliers").select("supplier_id")
                  .withColumn("s_idx", F.row_number().over(Window.orderBy("supplier_id")) - 1))
parts_idx      = (spark.table(f"{TEMP_DB}.parts").select("part_id", "unit_cost_usd")
                  .withColumn("p_idx", F.row_number().over(Window.orderBy("part_id")) - 1))
facilities_idx = (spark.table(f"{TEMP_DB}.facilities").select("facility_id")
...
status_expr   = (F.when(F.col("_rs") < 0.20, "Open").when(F.col("_rs") < 0.50, "In-Transit")
                  .when(F.col("_rs") < 0.85, "Received").when(F.col("_rs") < 0.92, "Delayed").otherwise("Cancelled"))
priority_expr = (F.when(F.col("_rpr") < 0.10, "Critical").when(F.col("_rpr") < 0.30, "High")
                  .when(F.col("_rpr") < 0.70, "Medium").otherwise("Low"))
quantity_expr = (F.when(F.col("unit_cost_usd") < 50,  (F.col("_rq") * 490 + 10).cast("int"))
                  .when(F.col("unit_cost_usd") < 500, (F.col("_rq") * 90  + 10).cast("int"))
                  .otherwise(                          (F.col("_rq") * 18  + 2).cast("int")))
lead_time_expr = (F.when(F.col("unit_cost_usd") < 50, (F.rand(seed=37) * 14 + 3).cast("int"))
...
po_df = (
    ...
    .withColumn("po_id",               F.concat(F.lit("PO-"), F.lpad(F.col("id").cast("string"), 7, "0")))
    .withColumn("quantity",            quantity_expr)
    .withColumn("unit_price_usd",      F.round(F.col("unit_cost_usd") * (1.0 + F.rand(seed=36) * 0.20), 2))
    .withColumn("total_value_usd",     F.round(F.col("quantity").cast("double") * F.col("unit_price_usd"), 2))
    .withColumn("order_date",          F.date_add(F.lit(str(START_DATE)).cast("date"), (F.col("_rd") * DATE_RANGE).cast("int")))
    .withColumn("lead_time_days",      lead_time_expr)
    .withColumn("expected_delivery_date", F.date_add(F.col("order_date"), F.col("lead_time_days")))
    ...
    .select("po_id", "facility_id", "supplier_id", "part_id",
            ...
            "order_date", "expected_delivery_date", "actual_delivery_date", "status", "priority")
)
...
print(f"purchase_orders: {spark.table(f'{TEMP_DB}.purchase_orders').count():,} rows → {VOLUME_PATH}/purchase_orders")

# COMMAND ----------

# MAGIC %md ## 6. Shipments (~12,000 rows)
...
shipments_df = (
    ...
    .withColumn("quantity_shipped",
        F.when(F.col("n_shipments") == 2, (F.col("quantity") * (F.rand(seed=43) * 0.40 + 0.30)).cast("int"))
         .otherwise(F.col("quantity")))
    .withColumn("ship_date",           F.date_add(F.col("order_date"), (F.rand(seed=44) * 5 + 1).cast("int")))
    .withColumn("transit_days",        F.datediff(F.col("expected_delivery_date"), F.col("order_date")))
    .withColumn("expected_arrival_date", F.date_add(F.col("ship_date"), F.col("transit_days")))
    .withColumn("status",              shipment_status_expr)
    .withColumn("delay_days",
        F.when(F.col("status") == "Delayed", (F.rand(seed=45) * 30 + 1).cast("int")).otherwise(F.lit(0)))
    ...
    .withColumn("shipment_id",
        F.concat(F.lit("SHP-"), F.lpad(
            F.row_number().over(Window.orderBy("po_id", "ship_num")).cast("string"), 8, "0")))
...
print(f"shipments: written → {VOLUME_PATH}/shipments")

# COMMAND ----------

# MAGIC %md ## Cleanup & Summary
...
# Verify output
for table in ["suppliers", "parts", "facilities", "bom", "purchase_orders", "shipments"]:
    files = dbutils.fs.ls(f"{VOLUME_PATH}/{table}")
    parquet_files = [f for f in files if f.name.endswith(".parquet")]
    print(f"  {table:<20} {len(parquet_files)} parquet file(s)")

print(f"\nData ready at: {VOLUME_PATH}/")
print("Next: databricks bundle run supply_chain_pipeline")
